...
import { translateError, base64ToFile } from "@doran/react";
...
const trans = "bannerEdit.bannerEdit";
...
class BannerEdit extends DoranForm {
    state = {
    ...
    handleSave = async (event) => {
        ...
        const { id, title, targetLink, categories } = this.state.data;
        const file = `/api/files/${image.id}`;
        const category = categories.map((e) => `/api/categories/${e.value}`);
        await bannerService
            .update(id, { title, targetLink, file, category })
            .then(({ data }) => {
                ...
                <Grid item sm={8}>
                    ...
                    <Grid item sm={12} className={classes.topBottom}>
                        {this.renderMultiSelectAutocomplete({
                            ...
                            listItemCreateCallback: this.listItemCreateCallback
                        })}
                    </Grid>
                </Grid>
                <Grid item sm={4}>
...
export default withTranslation("banner")(withStyles(styles)(BannerEdit));
