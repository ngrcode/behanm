...
const trans = "categories.list.categoryColumns";

export default function RenderColumns(props) {
    const { t: translate, onDelete, onEdit } = props;
    ...
    const columns = [
        ...
        { key: "body", label: translate(`${trans}.body`), content: (item) => item.body, width: "40%" },
        {
            key: "type",
            label: translate(`${trans}.type`),
            content: (item) => categoryTypes.find((e) => e.value === item.type).label,
            width: "20%"
        },
        {
            key: "extra",
